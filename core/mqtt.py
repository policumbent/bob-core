...
class Mqtt:
    _DEFAULT_HOSTNAME = "server.policumbent.it"
    _DEFAULT_PORT = 1883
...
async def main():
    ...
    # NOTE: esempio publisher
    async with Mqtt(
        "server.policumbent.it",
        port=1883,
        username="",
        password="",
    ) as client:
        ...
        cnt = 1
        while True:
            message = f"hello from bob-core {cnt}"
            cnt += 1
            await client.publish("test_topic/ciao", message)

            await asyncio.sleep(1)

    # NOTE: esempio listener
    async with Mqtt(
        "server.policumbent.it",
        port=1883,
        username="",
        ...
        message_loop = await client.subscribe("test_topic")
